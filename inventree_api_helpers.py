...
try:
    ...
    log.warning("Could not import SupplierPart/Company related classes. Supplier checks might be limited.")
...
def get_part_details(_api: InvenTreeAPI, part_id: int) -> Optional[Dict[str, any]]:
    ...
    try:
        ...
        if not part or not hasattr(part, '_data') or not part._data:
            log.warning(f"Could not retrieve valid part details for ID {part_id} from API.")
            return None
...
def get_bom_items(_api: InvenTreeAPI, part_id: int) -> Optional[List[Dict[str, any]]]:
    ...
    try:
        ...
        if not part or not hasattr(part, '_data') or not part._data:
             log.warning(f"Could not retrieve valid part object for BOM fetch for ID {part_id}.")
            ...
            bom_data = [
                {
                    ...
                    "allow_variants": bool(getattr(item, "allow_variants", True)), # Assume True if attr missing
                }
...
def get_parts_in_category(
    ...
    try:
        ...
        result_list = [
            ...
            for part in parts_list if part.pk and part.name
        ]
...
def get_final_part_data(_api: InvenTreeAPI, part_ids: Tuple[int, ...]) -> Dict[int, Dict[str, any]]:
    ...
    log.info(f"Fetching final details (incl. manufacturer, suppliers) for {len(part_ids_list)} base components...")
    ...
    try:
        if not _api:
            ...
            for part_id in part_ids_list: final_data[part_id] = get_default_data(part_id)
            return final_data

        # Fetch base fields including manufacturer name
        # Requesting 'pk' ensures we get Part objects back, not just dicts
        parts_details_list = list(Part.list(_api, pk__in=part_ids_list, fields=['pk', 'name', 'in_stock', 'is_template', 'variant_stock', 'manufacturer_name']))

        if parts_details_list:
            for part in parts_details_list:
                if not part or not part.pk or not hasattr(part, '_data') or not part._data:
                    log.warning(f"Received invalid part object for ID {part.pk if part else 'N/A'} during batch fetch.")
                    if part and part.pk and part.pk not in final_data: # Add default if not already added by missed_ids
            ...
            if missed_ids:
                log.warning(f"Could not fetch base details for some part IDs: {missed_ids}")
        ...
        log.error(f"Error fetching batch base part data: {e}. Returning defaults.", exc_info=True)
        for part_id in part_ids_list:
            if part_id not in final_data: final_data[part_id] = get_default_data(part_id)
    ...
    if IMPORTS_AVAILABLE and part_objects: # Only proceed if imports worked and we have parts
        part_ids_to_fetch_suppliers = list(part_objects.keys())
        CHUNK_SIZE = 100 # Define a chunk size for API calls
        log.info(f"Batch fetching supplier information for {len(part_ids_to_fetch_suppliers)} parts in chunks of {CHUNK_SIZE}...")
        ...
        try:
            ...
            for id_chunk in _chunk_list(part_ids_to_fetch_suppliers, CHUNK_SIZE):
                 log.debug(f"Fetching SupplierParts for chunk of {len(id_chunk)} part IDs...")
                 chunk_supplier_parts = SupplierPart.list(_api, part__in=id_chunk, fields=['pk', 'part', 'supplier', 'SKU'])
                 if chunk_supplier_parts:
                     ...
                     for sp in chunk_supplier_parts:
                         ...
                         if sp.supplier:
                             all_supplier_pks.add(sp.supplier) # Collect unique Company PKs
                 # Add a small delay or check API rate limits if needed, though unlikely for internal server

            if total_sps_fetched > 0:
                 log.info(f"Fetched a total of {total_sps_fetched} SupplierPart links across all chunks.")
            else:
                 log.info("No SupplierPart links found for the requested parts across all chunks.")
        ...
        if all_supplier_pks and not supplier_part_fetch_error:
            log.info(f"Batch fetching names for {len(all_supplier_pks)} unique suppliers in chunks of {CHUNK_SIZE}...")
            ...
            try:
                ...
                for pk_chunk in _chunk_list(supplier_pks_list, CHUNK_SIZE):
                    log.debug(f"Fetching Company names for chunk of {len(pk_chunk)} PKs...")
                    chunk_companies = Company.list(_api, pk__in=pk_chunk, fields=['pk', 'name'])
                    if chunk_companies:
                        for comp in chunk_companies:
                             if comp and comp.pk and comp.name:
                                 company_pk_to_name[comp.pk] = comp.name
                log.info(f"Fetched names for {len(company_pk_to_name)} suppliers across all chunks.")
            except Exception as e:
                 log.error(f"Error during chunked Company name fetch: {e}", exc_info=True)
        ...
        for part_id in part_ids_to_fetch_suppliers:
            ...
            if part_id in supplier_parts_map:
                for sp in supplier_parts_map[part_id]:
                    ...
                    if supplier_name:
                        ...
                        log.debug(f"Part ID {part_id}: Mapped supplier '{supplier_name}' via SupplierPart PK {sp.pk}")
                    elif supplier_pk:
                         log.debug(f"Part ID {part_id}: SupplierPart PK {sp.pk} linked to Company PK {supplier_pk}, but name not found in batch result.")
            ...
            if part_id in final_data:
                 ...
                 log.warning(f"Part ID {part_id} was in fetch list but not in final_data. Setting default supplier names.")
                 final_data[part_id] = get_default_data(part_id) # Add default if missing
                 final_data[part_id]["supplier_names"] = sorted(list(names))

        log.info("Finished mapping supplier names.")

    elif not IMPORTS_AVAILABLE:
        log.warning("Skipping supplier info fetch because necessary classes could not be imported.")
