...
def add_assembly_input(default_part_id: Optional[int]) -> None:
    ...
    if default_part_id:
        ...
        st.session_state.target_assemblies.append({"id": default_part_id, "quantity": 1})
    else:
...
def remove_assembly_row(index_to_remove: int) -> None:
    ...
    if "target_assemblies" in st.session_state and 0 <= index_to_remove < len(st.session_state.target_assemblies):
...
def render_assembly_inputs(
    ...
    if "target_assemblies" not in st.session_state:
        if default_part_id:
            st.session_state.target_assemblies = [{"id": default_part_id, "quantity": 1}]
        else:
    ...
    if not part_names:
        ...
        for i in indices_to_render:
             ...
             cols = st.sidebar.columns([0.5, 0.3, 0.2]) # Selectbox, Number Input, Remove Button
             selected_name = None
             new_qty = None
...
def render_results_table(results_list: Optional[List[Dict[str, Any]]]) -> None:
    ...
    if results_list is not None:
        if len(results_list) > 0:
            ...
            df_full['Part URL'] = df_full['pk'].apply(lambda pk: f"{base_url}platform/part/{pk}/")
            ...
            df_display = df_full[[col for col in display_columns_ordered if col in df_full.columns]] # Select only existing columns
            ...
            df_csv = df_full[[col for col in csv_columns_ordered if col in df_full.columns]]
            ...
            st.info("👍 Alle Teile auf Lager oder Filterkriterien erfüllen keine Teile.")
    else:
