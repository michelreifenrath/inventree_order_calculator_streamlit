...
PO_STATUS_MAP = {10: "Pending", 20: "In Progress", 25: "Placed", 30: "Complete", 40: "Cancelled", 50: "Lost", 60: "Returned", 70: "On Hold"}
...
try:
    ...
    found_po = False
    if all_pos:
        logging.info(f"Fetched {len(all_pos)} total POs. Searching for {TARGET_PO_REF}...")
        for po in all_pos:
            ref = po._data.get('reference')
            if ref == TARGET_PO_REF:
                pk = po.pk
                status_code = po._data.get('status')
                status_label = PO_STATUS_MAP.get(status_code, f"Unknown Code ({status_code})")
        ...
        if not found_po:
            logging.warning(f"Target PO reference '{TARGET_PO_REF}' not found in the fetched list of {len(all_pos)} POs.")
    else:
