...
def get_recursive_bom_cached(api: InvenTreeAPI, part_id: int, quantity: float, required_components: defaultdict[int, float]):
    """
    ...
    if part_details['assembly']:
        ...
        if bom_items: # Check if BOM fetch succeeded and is not empty
            for item in bom_items:
                ...
                get_recursive_bom_cached(api, sub_part_id, total_sub_quantity, required_components)
...
def get_final_part_data(api: InvenTreeAPI, part_ids: list[int]) -> dict[int, dict]:
    ...
    log.info(f"Fetching final details (name, stock) for {len(part_ids)} base components...")
    try:
        ...
        if parts_details:
            ...
            log.warning("pk__in filter returned no results for final data fetch. This might indicate issues.")
...
def save_results_to_markdown(parts_to_order: list[dict], filename: str):
    ...
    try:
        ...
        if not parts_to_order:
            ...
            # Create table header
            md_content.append("| PK   | Name                                               | Required | In Stock | To Order |")
            md_content.append("| :--- | :------------------------------------------------- | :------- | :------- | :------- |")
            # Add table rows
            for item in parts_to_order:
                ...
                md_content.append(f"| {item['pk']:<4} | {name_escaped:<50} | {req_str:<8} | {stock_str:<8} | {order_str:<8} |")
        ...
        log.error(f"Error saving results to Markdown file {filename}: {e}", exc_info=True)
...
def main():
    ...
    if not INVENTREE_URL or not INVENTREE_TOKEN:
        log.error("INVENTREE_URL and INVENTREE_TOKEN environment variables must be set.")
        sys.exit(1)
    ...
    for part_id, quantity in TARGET_ASSEMBLY_IDS.items(): # Use the ID-based dictionary directly
        log.info(f"Processing target assembly ID: {part_id}, Quantity: {quantity}")
        get_recursive_bom_cached(api, part_id, float(quantity), required_base_components)
    ...
    for part_id, required_qty in required_base_components.items():
        part_data = final_part_data.get(part_id, {'name': f"Unknown (ID: {part_id})", 'in_stock': 0.0})
    ...
    if not parts_to_order:
        ...
        # Simple print format
        print(f"{'PK':<6} | {'Name':<50} | {'Required':<10} | {'In Stock':<10} | {'To Order':<10}")
        print("-" * 90)
        for item in parts_to_order:
             # Format floats for printing
             req_str = f"{item['required']:.3f}".rstrip('0').rstrip('.')
             stock_str = f"{item['in_stock']:.3f}".rstrip('0').rstrip('.')
             order_str = f"{item['to_order']:.3f}".rstrip('0').rstrip('.')
             print(f"{item['pk']:<6} | {item['name']:<50} | {req_str:<10} | {stock_str:<10} | {order_str:<10}")
        print("-" * 90)
